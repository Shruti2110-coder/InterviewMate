...
const Interview = () => {
    ...
    // useInterview already fetches by :interviewId - a second effect here made
    // every page load request the report twice.
    ...
    // Without this the page spun forever on any failed fetch.
    if (error || !report) {
        return (
            <main className='loading-screen'>
                <h1>{error || 'This interview plan could not be found.'}</h1>
                <button className='button primary-button' onClick={() => navigate('/')}>Back to my plans</button>
            </main>
        )
    }
    ...
    const scoreLabel =
        matchScore >= 80 ? 'Strong match for this role' :
            matchScore >= 60 ? 'Partial match for this role' : 'Weak match for this role'
